      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl relative text-white">
        ...
        {step === 'login' ? (
          <div>
            ...
            {/* Login Method Tabs */}
            <div className="flex bg-slate-950 p-1 rounded-2xl border border-slate-800 mb-4 text-xs font-bold">
              <button
                type="button"
                onClick={() => setLoginMethod('google')}
                className={`flex-1 py-2 rounded-xl transition-all ${
                  loginMethod === 'google' ? 'bg-emerald-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'
                }`}
              >
                🌐 Google
              </button>
            ...
            {loginMethod === 'google' && (
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center space-x-3 bg-white hover:bg-slate-100 text-slate-900 font-bold py-3.5 px-4 rounded-xl shadow-lg transition-all hover:scale-[1.01] active:scale-[0.99] mb-4 disabled:opacity-50"
              >
              ...
              <form onSubmit={handleEmailSignIn} className="space-y-3 mb-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. hapa1929@gmail.com"
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold py-3 rounded-xl text-xs shadow-lg shadow-emerald-500/20 transition-all hover:scale-105"
                >
            ...
            {loginMethod === 'phone' && (
              <form
                onSubmit={(e) => {
                  ...
                  const isOwner = emailInput.toLowerCase() === ADMIN_EMAIL.toLowerCase();
                  setTempUser({
                    name: 'Verified Traveler',
                    email: isOwner ? ADMIN_EMAIL : 'traveler@wandervibe.com',
                    phone: phoneNumber,
                    isAdmin: isOwner
                  });
                  setStep('phone_prompt');
              ...
              >
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1 flex items-center space-x-1">
                    ...
                    <span>Mobile Number (For Instant SMS Verification)</span>
                  </label>
                  <input
                    type="tel"
                    required
            ...
            <div className="mt-6 flex items-start space-x-2.5 bg-slate-950/80 p-3.5 rounded-2xl border border-slate-800 text-xs text-slate-400">
              ...
              <p className="text-[11px] leading-relaxed">
                WanderVibe is a verified travel agency. Authenticate securely via Google to manage your trip bookings, digital passes, and agency control panel.
              </p>
            </div>
          </div>
        ) : (
